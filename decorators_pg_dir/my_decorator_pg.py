...
def check_if_number(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # To add clarity and feedback in case of wrong doing. Will raise AssertionError if False.
        assert len(addition_tuple) == 2, "Should only be two elements in tuple!"  

        # Checks if arguments are numbers
        check_res = list(map(find_int_or_float.is_number, addition_tuple))  # If one of the elements is not a number, raise ValueError
        for res in check_res:
            if not res[2]:  # If one of the elements is not a number, raise ValueError
                raise ValueError
